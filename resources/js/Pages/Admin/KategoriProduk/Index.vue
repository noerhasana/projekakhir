...
function fetchData(params) {
    ...
    router.visit(route('kategori-produk.index', params), {
        only: [
            'data',
        ],
    })
...
function showDetail(id) {
    router.visit(route('kategori-produk.show', {id: id}));
}

function edit(id) {
    router.visit(route('kategori-produk.edit', {id: id}));
}
...
function confirmDelete(id) {
    confirm.require({
        ...
        accept: () => {
            router.delete(route('kategori-produk.destroy', {id: id}), {
                onFinish: visit => {
                    toast.add({ 
...
<template>
    ...
    <AuthenticatedLayout>
        <template #name>Kategori Produk</template>
        
        <div class="mb-8">
            <Link :href="route('kategori-produk.create')">
            ...
            <div class="mt-10">
                    ...
                    <div class="w-1/2 lg:w-96">
                        <form @submit.prevent="search">
                            ...
                            <div class="p-inputgroup">
                                ...
                                <Button type="submit" label="Cari" icon="fas fa-magnifying-glass" />
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="bg-white border rounded-lg p-1 my-2">
                    <DataTable stripedRows :value="data.data" :rows="meta.total">
                        <Column field="nama" header="Kategori" />
